...
```{r}
#Plot literal response rate for do vs. po and implausible vs. plausible
ggplot(both_literal, aes(x = do_po, y = literal_response_rate, fill = plausibility)) +
...
# Lmer model
contrasts(implausible_data$do_po) <- c(-.5, .5)
#contrasts(data$plausibility) <- c(-.5, .5)
#Contrast the is_literal variable
contrasts(implausible_data$is_literal) <- c(0,1)

#Since our result is categorical, we need to do a logistic regression: include intercepts and slopes
model <- glmer(is_literal ~ do_po + (1|item) , 
data = implausible_data, family = binomial(link = "logit"))
